    ...
    server.route({
        ...
        config:{
            ...
            handler: (request, reply) => {
                ...
                db.collection('arbitros').find( { 'pass':request.params.pass,'userName':request.params.name }).toArray((err, result) => {//toAarray si no tenemos un objeto cursor que no puede devolver reply
                    if (err) {
