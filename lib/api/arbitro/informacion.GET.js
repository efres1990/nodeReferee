...
internals.after = (server, next) => {
    ...
    server.route({
        method: 'GET',
        path: '/football/info/{arbitro}',
        config:{
            description:'obtener informacion del arbitro',
            handler: (request, reply) => {

                const db = server.plugins['hapi-mongodb'].db;//tb var db = request.server.plugins['hapi-mongodb'].db[0];
                db.collection('arbitros').find( { 'name':request.params.arbitro }).toArray((err, result) => {//toAarray si no tenemos un objeto cursor que no puede devolver reply
                    if (err) {
